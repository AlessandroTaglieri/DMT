...
def __print_top_k(value_max_probs, index_max_probs, vocab, mask_topk, index_list, max_printouts = 10):
    j = 0
    result = []
    df = pd.DataFrame(columns=['position', 'prediction', 'prob'])
    ...
    for i in range(mask_topk):
        ...
        if index_list is not None:
            # the softmax
            #layer has been filtered using the vocab_subset
...
def get_ranking(log_probs, masked_indices, vocab, label_index = None, index_list = None, topk = 1000, P_AT = 10, print_generation=True):
    ...
    result_masked_topk, return_msg, df = __print_top_k(value_max_probs, index_max_probs, vocab, topk, index_list)
    ...
    return MRR, P_AT_X, experiment_result, return_msg, df
